...
def gui_number(draw, minimum, maximum, size, x, y, index, lock, id, box1, box2):
    selection = wrap(index, lock)
    selected = (selection == id)
    font = ImageFont.truetype("font.ttf", size)
    ...
    while True:
        ...
        with canvas(device) as draw:
            ...
            if page == "auto":
                ...
                gui_number(draw, 0, 30, 20, 25, 25, value, 3, 2, 55, 85) 
        time.sleep(0.05)
...
def button_callback():
    ...
    if not button_handled:  # Only process if not already handled
        pressed = 1
        button_handled = True  # Mark as handled
